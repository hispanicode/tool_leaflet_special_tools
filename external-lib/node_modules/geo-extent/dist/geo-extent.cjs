...
exports.GeoExtent = void 0;

var _add = _interopRequireDefault(require("preciso/add.js"));

var _divide = _interopRequireDefault(require("preciso/divide.js"));

var _multiply = _interopRequireDefault(require("preciso/multiply.js"));

var _subtract = _interopRequireDefault(require("preciso/subtract.js"));
...
const avg = (a, b) => (0, _divide.default)((0, _add.default)(a.toString(), b.toString()), "2");

const isAry = o => Array.isArray(o);

const isDef = o => o !== undefined && o !== null && o !== "";

const isFunc = o => typeof o === "function"; // identifying GeoJSON currently unused
...
const normalize = srs => {
  ...
  if (isStr(srs) && srs.match(/^\d+$/)) return "EPSG:" + srs;else if (isNum(srs)) return "EPSG:" + srs;
  const code = (0, _getEpsgCode.default)(srs);
...
class GeoExtent {
  constructor(o, {
    ...
    if (isAry(o) && o.length === 4 && allNums(o)) {
      ...
      ({
        lat: ymin,
        lng: xmin
      } = o._bounds._southWest);
      ({
        lat: ymax,
        lng: xmax
      } = o._bounds._northEast);
      if (!isDef(this.srs)) this.srs = "EPSG:4326";
    } else {
    ...
    this.width_str = (0, _subtract.default)(this.xmax_str, this.xmin_str);
    this.width = Number(this.width_str);
    this.height_str = (0, _subtract.default)(this.ymax_str, this.ymin_str);
    this.height = Number(this.height_str); // corners
    ...
    this.leafletBounds = [[this.ymin, this.xmin], [this.ymax, this.xmax]];
    this.area_str = (0, _multiply.default)(this.width_str, this.height_str);
    this.area = Number(this.area_str);
    this.perimeter_str = (0, _add.default)((0, _multiply.default)(this.width_str, "2"), (0, _multiply.default)(this.height_str, "2"));
    ...
    this.center = {
      x: Number(this.center_str.x),
      y: Number(this.center_str.y)
    };
  ...
  _pre(_this, _other) {
    ...
    if (!isDef(_this.srs) && !isDef(_other.srs)) {// assume same/no projection
    } else if (isDef(_this.srs) && !isDef(_other.srs)) {
      // assume other is the same srs as this
      _other = new _this.constructor({ ..._other,
        srs: _this.srs
      });
    } else if (!isDef(_this.srs) && isDef(_other.srs)) {
      // assume this' srs is the same as other
      _this = new _this.constructor({ ..._this,
        srs: _other.srs
      });
    } else if (isDef(_this.srs) && isDef(_other.srs) && _this.srs !== _other.srs) {
      _other = _other.reproj(_this.srs);
    } else if (isDef(_this.srs) && isDef(_other.srs) && _this.srs === _other.srs) {// same projection, so no reprojection necessary
    } else {
      throw "UH OH";
    }
  ...
  crop(other) {
    other = new this.constructor(other); // if really no overlap then return null

    if (this.overlaps(other, {
      quiet: true
    }) === false && other.overlaps(this, {
      quiet: true
    }) === false) return null; // first check if other fully contains this extent
    // in which case, we don't really need to crop
    // and can just return the extent of this

    if (other.contains(this)) return this.clone(); // check if special case where other crosses 180th meridian

    if (other.srs === "EPSG:4326" && (other.xmin < -180 || other.xmax > 180)) {
      const parts = other.unwrap();
      let cropped = parts.map(it => this.crop(it)); // filter out any parts that are null (didn't overlap)

      cropped = cropped.filter(Boolean); // no overlap
    ...
    let another = isDef(this.srs) && isDef(other.srs) ? other.reproj(this.srs, {
      quiet: true
    }) : other.clone();
    ...
    if (another) {
      ...
      return new this.constructor([xmin, ymin, xmax, ymax], {
        srs: this.srs
      });
    ...
    return new this.constructor([minLon, minLat, maxLon, maxLat], {
      srs: 4326
    }).reproj(this.srs);
  ...
  */
  ...
  overlaps(other, {
    ...
    try {
      ...
      if (quiet) return;else throw error;
    }
  }
  ...
  reproj(to, {
    ...
    const reprojected = (0, _reprojectBbox.default)({
      bbox: this.bbox,
      from: this.srs,
      to
    });
    ...
    return new GeoExtent(reprojected, {
      srs: to
    });
  }
  ...
  unwrap() {
    const {
      ...
      ymax,
      srs
    } = this; // not in 4326, so just return a clone

    if (srs !== "EPSG:4326") return [this.clone()]; // extent is within the normal extent of the earth, so return clone

    if (xmin > -180 && xmax < 180) return [this.clone()]; // handle special case where extent overflows xmin and then overlaps itself

    if (xmin < -180 && xmax >= xmin + 360) return [new this.constructor([-180, ymin, 180, ymax], {
      srs: 4326
    })];
    if (xmax > 180 && xmin <= xmax - 360) return [new this.constructor([-180, ymin, 180, ymax], {
      srs: 4326
    })];
    let extents = []; // extent overflows left edge of the world

    if (xmin < -180) {
      extents.push(new this.constructor([xmin + 360, ymin, 180, ymax], {
        srs
      }));
    } // add extent for part between -180 to 180 longitude


    extents.push(new this.constructor([xmin < -180 ? -180 : xmin, ymin, xmax > 180 ? 180 : xmax, ymax], {
      srs
    })); // extent overflows right edge of the world

    if (this.xmax > 180) {
      extents.push(new this.constructor([-180, ymin, xmax - 360, ymax], {
        srs
      }));
    }
  ...
  asGeoJSON() {
    const {
      ...
      xmax,
      ymax
    } = this.srs === "EPSG:4326" ? this : this.reproj(4326);
    return {
      type: "Feature",
      geometry: {
        type: "Polygon",
