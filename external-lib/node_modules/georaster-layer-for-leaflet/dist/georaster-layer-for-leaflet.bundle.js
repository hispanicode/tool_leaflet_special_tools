...
var _geocanvas = _interopRequireDefault(require("geocanvas"));
...
const GeoRasterLayer = L.GridLayer.extend({
  ...
  createTile: function (coords, done) {
    ...
    const tile = L.DomUtil.create("canvas", "leaflet-tile"); // we do this because sometimes css normalizers will set * to box-sizing: border-box

    tile.style.boxSizing = "content-box"; // start tile hidden

    tile.style.visibility = "hidden";
    const context = tile.getContext("2d"); // note that we aren't setting the tile height or width here
  ...
  drawTile: function ({
    ...
    try {
      ...
      const {
        z: zoom
      } = coords; // stringified hash of tile coordinates for caching purposes
      ...
      const extentOfLayer = new _geoExtent.GeoExtent(this.getBounds(), {
      ...
      const extentOfTile = new _geoExtent.GeoExtent(boundsOfTile, {
        srs: inSimpleCRS ? "simple" : 4326
      });
      if (debugLevel >= 2) log({
        extentOfTile
      ...
      const extentOfInnerTileInRasterCRS = new _geoExtent.GeoExtent(snapped.bbox_in_coordinate_system, {
      ...
      {
        const oldrb = new _geoExtent.GeoExtent(oldExtentOfInnerTileInRasterCRS.bbox);
        const newrb = new _geoExtent.GeoExtent(extentOfInnerTileInRasterCRS.bbox);
        const oldmb = new _geoExtent.GeoExtent(extentOfInnerTileInMapCRS.bbox);
      ...
      const canvasWidth = this.tileWidth - canvasPadding.left - canvasPadding.right; // set padding and size of canvas tile
      ...
      const tileSize = this.getTileSize(); // this converts tile coordinates (how many tiles down and right)
            ...
            if (lat > yMinOfLayer && lat < yMaxOfLayer) {
              ...
              for (let w = 0; w < numberOfSamplesAcross; w++) {
                ...
                if (xOfLayer > xMinOfLayer && xOfLayer < xMaxOfLayer) {
                  ...
                  const x = Math.round(w * widthOfSampleInScreenPixels) + Math.min(padding.left, 0); // y-axis coordinate of the starting point of the rectangle representing the raster pixel

                  const y = yInTilePixels; // how many real screen pixels does a pixel of the sampled raster take up
  ...
  _isValidTile: function (coords) {
    ...
    const {
      ...
      y,
      z
    } = coords; // not sure what srs should be here when simple crs

    const layerExtent = new _geoExtent.GeoExtent(bounds, {
      srs: 4326
    });

    const boundsOfTile = this._tileCoordsToBounds(coords); // check given tile coordinates
    ...
    const width = Math.pow(2, z); // check one world to the left
  ...
  getActiveTiles() {
    const tiles = this.getTiles(); // only return valid tiles
  ...
  isSupportedProjection: function () {
    if (this._isSupportedProjection === undefined) {
      ...
      if ((0, _isUTM.default)(projection)) {
        ...
        this._isSupportedProjection = true;
      } else if (typeof _proj4FullyLoaded.default === "function" && `EPSG:${projection}` in _proj4FullyLoaded.default.defs) {
        this._isSupportedProjection = true;
  ...
  initBounds: function (options) {
    ...
    if (!this._bounds) {
      const {
        ...
        ymin,
        ymax
      } = this; // check if map using Simple CRS
